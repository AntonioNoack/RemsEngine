...
fun main() {
    ...
    if (false) {
        ...
        GLTFWriter().write(
            PrefabCache[sceneMain]!!, desktop.getChild("$name.glb"),
            callback
        )
    } else if (false) {
        // test for non-packed references
        GLTFWriter().write(
            PrefabCache[documents.getChild("cube bricks.fbx")]!!, desktop.getChild("bricks.glb"),
            callback
        )
    } else {
        // test for vertex colors
        GLTFWriter().write(
            PrefabCache[downloads.getChild("3d/seal gltf/scene.gltf")]!!, desktop.getChild("seal.glb"),
            callback
        )
    }
}