...
xcf_load_image(Gimp *gimp,
    ...
    if (parasite) {
        ...
        if (has_metadata) {
            g_printerr(
                "xcf-load: inconsistent metadata discovered: XCF file "
    ...
    while (TRUE) {
        ...
        if (layer != info->floating_sel) {
            ...
            if (item_path) {
                ...
                parent = GIMP_LAYER(gimp_item_stack_get_parent_by_path(GIMP_ITEM_STACK(layers),
                                                                       item_path,
                                                                       NULL));
        ...
        switch (prop_type) {
            ...
            case PROP_OFFSETS: {
                ...
                if (offset_x < -GIMP_MAX_IMAGE_SIZE ||
                    ...
                    g_printerr(
                        "unexpected item offset_x (%d) in XCF, "
                        "setting to 0\n",
                        offset_x);
                    offset_x = 0;
                }
                ...
                if (offset_y < -GIMP_MAX_IMAGE_SIZE ||
                    ...
                    g_printerr(
                        "unexpected item offset_y (%d) in XCF, "
                        "setting to 0\n",
                        offset_y);
                    offset_y = 0;
                }
    ...
    for (i = 0; i < ntiles; i++) {
        ...
        switch (info->compression) {
            ...
            case COMPRESS_FRACTAL:
                g_printerr(
                    "xcf: fractal compression unimplemented. "
                    "Possibly corrupt XCF file.");
                fail = TRUE;
                break;
            default:
                g_printerr(
                    "xcf: unknown compression. "
                    "Possibly corrupt XCF file.");
                fail = TRUE;
                break;
        }
...
static GimpParasite *xcf_load_parasite(XcfInfo *info) {
    ...
    if (size > MAX_XCF_PARASITE_DATA_LEN) {
        g_printerr(
            "Maximum parasite data length (%ld bytes) exceeded. "
            "Possibly corrupt XCF file.",
...
#ifdef GIMP_XCF_PATH_DEBUG
    g_printerr(
        "name: %s, tattoo: %d, visible: %d, linked: %d, "
