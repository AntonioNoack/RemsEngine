...
class LuaTokenizer(var customVariables: Set<String> = emptySet()) : LanguageTokenizer {
    ...
    companion object {
        ...
        @JvmStatic
        private val builtIns0 = "" +
                "_G,_VERSION,assert,collectgarbage,dofile,error,getfenv,getmetatable,ipairs,load,loadfile," +
                ...
                "math.cosh,math.deg,math.exp,math.floor,math.fmod,math.frexp,math.huge,math.ldexp," +
                "math.log,math.log10,math.max,math.min,math.modf,math.pi,math.pow,math.rad," +
        ...
        @JvmStatic
        private val keywords0 = "and,break,elseif,false,nil,not,or,return,true,function," +
                "end,if,then,else,do,while,repeat,until,for,in,local"

        @JvmStatic
        private val builtIns = LanguageTokenizer.fullMatch(builtIns0.split(','))

        @JvmStatic
        private val keywords = LanguageTokenizer.fullMatch(keywords0.split(','))

        @JvmStatic
        private val indentTokens = LanguageTokenizer.fullMatch("function,if,repeat,do,\\(,{".split(','))

        @JvmStatic
        private val dedentTokens = LanguageTokenizer.fullMatch("end,until,\\),}".split(','))

        @JvmStatic
        private val dedentPartial = LanguageTokenizer.partialMatch("end,until,\\),},else,elseif".split(','))
    }
