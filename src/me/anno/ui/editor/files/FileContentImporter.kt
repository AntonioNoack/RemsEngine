...
open class FileContentImporter<V> {
    ...
    fun addChildFromFile(
        parent: V?, file: FileReference,
        useSoftLink: SoftLinkMode, doSelect: Boolean, callback: (V) -> Unit
    ) = addChildFromFile(parent, file, useSoftLink, doSelect, 0, callback)

    fun addChildFromFile(
        parent: V?, file: FileReference,
        useSoftLink: SoftLinkMode, doSelect: Boolean,
        depth: Int, callback: (V) -> Unit
    ) {
        if (file.isDirectory) {
            ...
            if (depth < DefaultConfig["import.depth.max", 3]) {
                ...
                file.listChildren(Callback.onSuccess { children ->
