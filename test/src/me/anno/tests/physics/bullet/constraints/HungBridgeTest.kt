...
fun main() {
    ...
    val pillarSideSize = Vector3d(0.2, height, 1.5)
    ...
    fun addPillar(i: Int, sign: Int) {
        ...
        fun addPillarSide(sign2: Int) {
            Entity("PillarSide[$sign,$sign2]", entity)
                .setPosition(
                    sign2 * (pillarSize.x - pillarSideSize.x) * 0.5,
                    (pillarSideSize.y + pillarSize.y) * 0.5,
                    0.0
                )
                .add(BoxCollider().apply {
                    halfExtents.set(pillarSideSize).mul(0.5f)
                    roundness = 0.1f
                })
                .add(MeshComponent(pillarSideMesh, pillarMaterial))
        }
    ...
    createSpringConstraint(3, 2, +1)

    spawnFloor(scene)
    spawnSampleCubes(scene)

    testSceneWithUI("Hung Bridge", scene)
}
