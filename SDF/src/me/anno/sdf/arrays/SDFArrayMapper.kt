...
import me.anno.sdf.SDFComponent.Companion.mod
...
class SDFArrayMapper : PositionMapper() {
    ...
    override fun calcTransform(pos: Vector4f, seeds: IntArrayList) {
        val rep = cellSize
        val lim = count
        if (count.x != 1 && rep.x > 0f) pos.x = if (mirrorX) mod2M(
            pos.x,
            rep.x,
            lim.x
        ) else mod2(pos.x, rep.x, lim.x)
        if (count.y != 1 && rep.y > 0f) pos.y = if (mirrorY) mod2M(
            pos.y,
            rep.y,
            lim.y
        ) else mod2(pos.y, rep.y, lim.y)
        if (count.z != 1 && rep.z > 0f) pos.z = if (mirrorZ) mod2M(
            pos.z,
            rep.z,
            lim.z
        ) else mod2(pos.z, rep.z, lim.z)
    }
