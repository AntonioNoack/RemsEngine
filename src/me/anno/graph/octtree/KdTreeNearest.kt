...
object KdTreeNearest {
    fun <Point, Value> KdTree<Point, Value>.findNearest(
        query: Point, maxDistance: Double = Double.POSITIVE_INFINITY
    ): Value? {
        ...
        Recursion.processRecursive(this) { node, remaining ->
            ...
            val baseDistance = distanceToBounds(query, node.min, node.max)
            if (baseDistance < bestDist) {
                val children = node.values
                if (children != null) {
                    for (i in children.indices) {
                        val child = children[i]
                        val dist = distanceToValue(query, child)
                        if (dist < bestDist) {
                            bestDist = dist
                            bestValue = child
                        }
    ...
    fun <Point, Value> KdTree<Point, Value>.findNearest(
        query: Point, min: Point, max: Point,
        ...
        Recursion.processRecursive(this) { node, remaining ->
            ...
            if (baseDistance < bestDist) {
                ...
                if (children != null) {
                    ...
                    val right = node.right
                    if (left != null && right != null) {
                        val addLeft = left.overlapsBounds(min, max)
                        val addRight = right.overlapsBounds(min, max)
                        if (addLeft && addRight) {
                            val axisValue = get(query, node.axis)
                            val leftDist = get(node.min, node.axis) - axisValue
                            val rightDist = axisValue - get(node.max, node.axis)
