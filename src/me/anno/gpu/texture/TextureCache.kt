...
object TextureCache {
    ...
    fun hasImageOrCrashed(file: FileReference, timeout: Long, asyncGenerator: Boolean): Boolean {
        ...
        val entry = try {
            textures.getFileEntry(file, false, timeout, asyncGenerator, ::generateImageData)
        } catch (e: Exception) {
            e.printStackTrace()
            return true
        }
    ...
    operator fun get(file: FileReference, timeout: Long, asyncGenerator: Boolean): ITexture2D? {
        ...
        val imageData = textures.getFileEntry(file, false, timeout, asyncGenerator, ::generateImageData)
    ...
    private fun generateImageData(file: FileKey, result: AsyncCacheData<ITexture2D>) =
    ...
    fun getLUT(file: FileReference, timeoutMillis: Long = TextureCache.timeoutMillis): Texture3D? {
        val texture = textureLUTs.getFileEntry(file, false, timeoutMillis, true, TextureCache::generateLUT)
        return texture.value?.createdOrNull() as? Texture3D
    }

    private fun generateLUT(key: FileKey, result: AsyncCacheData<Texture3D>) {
        ImageCache.getAsync(key.file, timeoutMillis, true, result.map { img ->
    ...
    fun getTextureArray(
        ...
        val texture = textureArrays2.getEntry(key, timeoutMillis, true, TextureCache::generateTextureArray)
    ...
    fun getTextureArray(
        ...
        val texture = textureArrays1.getEntry(key, timeoutMillis, true, TextureCache::generateTextureArray)
