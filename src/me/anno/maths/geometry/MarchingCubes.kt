...
object MarchingCubes {
    ...
    // could be written to a file as well... but idk, it's not THAT long
    val edgeTable = shortArrayOf(
        0x109, 0x203, 0x30a, 0x406, 0x50f, 0x605, 0x70c, 0x80c, 0x905, 0xa0f, 0xb06, 0xc0a, 0xd03, 0xe09, 0xf00,
            ...
            for (y in 0 until h - 1) {
                ...
                for (x in 0 until w - 1) {
                    ...
                    if (code in 0 until 254) {
                        ...
                        val edgeMask = edgeTable[code].toInt()
    ...
    fun debugPolygons(w: Int, h: Int, values: FloatArray, triangles: List<Vector3f>) {
        ...
        ImageWriter.writeImageFloat(
            ...
            for (i in triangles.indices step 3) {
                ...
                for (idx in i until i + 3) {
                    val a = triangles[idx]
                    distanceSq = min(distanceSq, LinearSegment.signedDistanceSq(px, py, a.x, a.y, b.x, b.y))
                    b = a
                }
            }
    ...
    fun main(args: Array<String>) {
        ...
        val values = FloatArray(w * h * d) { i1 ->
            ...
            val x = xi - (w - 1f) / 2f
            val y = yi - (h - 1f) / 2f
            val z = zi - d * 1.5f // so we can see an effect
            (x * x + y * y + z * z) * 2f - t
            random.getValue(x, y, z * 0.2f) - 0.5f
        }
        val polygons = march(w, h, d, values, 0f, false)
