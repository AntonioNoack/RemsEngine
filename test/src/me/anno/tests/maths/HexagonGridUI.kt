...
fun main() {
    TestDrawPanel.testDrawing("Hexagon Grid") {
        ...
        val hovCell = HexagonGridMaths.coordsToIndexClosest(mouseCoords, Vector2i())
        val hovLine = HexagonGridMaths.getClosestLine(mouseCoords, Vector2d(), Vector2d(), Vector2i())
        val hovVert = HexagonGridMaths.getClosestVertex(mouseCoords, false, Vector2d(), Vector2d(), Vector2i())
        ...
        for (j in -dy..dy) {
            for (i in -dx..dx) {
                ...
                val color = white.withAlpha(if (isHovered) 255 else 100)
                val c = HexagonGridMaths.indexToCoords(i, j, tmp).mul(scale)
                val x = c.x
                val y = c.y
                DrawRectangles.drawRect(x.toInt() + cx - 1, y.toInt() + cy - 1, 3, 3, color)
                val a = HexagonGridMaths.getVertex(i, j, 0, tmp1).mul(scale)
                for (k in 0 until 6) {

                    val b = HexagonGridMaths.getVertex(i, j, (k + 1) % 6, tmp2).mul(scale)
                    val f = 0.05
