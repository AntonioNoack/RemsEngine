...
class MedianTest {
    ...
    fun <V> MutableList<V>.medianOfMedians(comparator: Comparator<V>): V {
        ...
        val medians = Lists.createArrayList(sqrtSize) {
            val i0 = WorkSplitter.Companion.partition(it, size, sqrtSize)
            val i1 = WorkSplitter.Companion.partition(it + 1, size, sqrtSize)
    ...
    fun medianOfMedians() {
        val n = (1 shl 10) + 1
        val elements = Lists.createArrayList(n) { it }
