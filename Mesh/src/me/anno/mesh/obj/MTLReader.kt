...
class MTLReader(val file: FileReference, input: InputStream) : TextFileReader(input) {
    ...
    private val materials = HashMap<String, Prefab>()

    private var hadOpacity = false
    private var color = Vector4f()
    private var material: Prefab? = null

    suspend fun read(): Map<String, Prefab> {
        // load all materials
    ...
    companion object {
        ...
        suspend fun readAsFolder(
            src: FileReference,
            dstFolder: InnerFolder = InnerFolder(src)
        ): Result<InnerFolder> {
            return src.inputStream().mapSuccess { it ->
                val materials = MTLReader(src, it).read()
                for ((name, material) in materials)
                    dstFolder.createPrefabChild("$name.json", material)
                Result.success(dstFolder)
            }
        }
    }
}